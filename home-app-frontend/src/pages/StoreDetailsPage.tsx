...
export function StoreDetailsPage() {
  ...
  const [fullscreenData, setFullscreenData] = useState<{ name: string, number: string, barcodeType: string } | null>(null)
  ...
  const updateCouponMutation = useMutation({
    mutationFn: ({ id, coupon }: { id: number; coupon: Partial<Coupon> }) => updateCoupon(id, coupon),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['coupons', storeId] })
  ...
  return (
    <Stack gap="lg">
      <Group>
        <Button variant="subtle" leftSection={<IconArrowLeft size={16} />} onClick={() => navigate('/shopping/stores')}>
          Back to Stores
        </Button>
      </Group>
      ...
      <Paper p="xl" radius="md" withBorder shadow="sm">
        <Group align="flex-start" wrap="nowrap">
          <Box w={80} h={80} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            {store.photo ? (
      ...
      <Tabs defaultValue="cards" variant="outline" radius="md">
          ...
          <Stack gap="md">
            ...
            <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="lg">
              {loyaltyCards?.map(card => (
                <Paper key={card.id} withBorder p="xl" radius="md" shadow="xs" style={{ overflow: 'hidden' }}>
                    ...
                    <Group justify="space-between" w="100%">
                      ...
                      <Group gap={4}>
                        ...
                        <ActionIcon variant="subtle" color="red" onClick={() => {
                          if (window.confirm('Delete this card?')) deleteCardMutation.mutate(card.id)
                        }}>
                          <IconTrash size={16} />
                        </ActionIcon>
                      </Group>
              ...
              {loyaltyCards?.length === 0 && (
                <Text ta="center" py="xl" c="dimmed">No loyalty cards found for this store.</Text>
              )}
            </SimpleGrid>
          </Stack>
        </Tabs.Panel>
        ...
        <Tabs.Panel value="coupons" pt="lg">
          <Stack gap="md">
            <Group justify="flex-end">
              <Button leftSection={<IconPlus size={16} />} onClick={() => { setEditingCoupon(null); couponForm.reset(); openCoupon(); }}>
                Add Coupon
              </Button>
            </Group>
      ...
      <Modal opened={cardOpened} onClose={closeCard} title="Add Loyalty Card" radius="md" zIndex={2000}>
        <form onSubmit={cardForm.onSubmit(v => addCardMutation.mutate(v))}>
          <Stack gap="md">
            ...
            />
            <Button type="submit" mt="md" loading={addCardMutation.isPending}>Add Card</Button>
          </Stack>
        </form>
      </Modal>
      ...
      <Modal 
        opened={couponOpened} 
        onClose={() => { closeCoupon(); setEditingCoupon(null); }} 
      ...
      >
        <form onSubmit={couponForm.onSubmit(handleCouponSubmit)}>
          <Stack gap="md">
            <TextInput required label="Coupon Name" placeholder="e.g. €5 off €25" {...couponForm.getInputProps('name')} />
            <TextInput 
              label="Value" 
              placeholder="5.00" 
            ...
            />
            <Textarea label="Description" placeholder="Optional details" {...couponForm.getInputProps('description')} />
            ...
            <Button type="submit" mt="md" loading={addCouponMutation.isPending || updateCouponMutation.isPending}>
            ...
            <Box bg="white" p="xl" style={{ borderRadius: rem(12), boxShadow: '0 0 20px rgba(0,0,0,0.1)' }}>
              {fullscreenData?.barcodeType === 'QR' ? (
                ...
                <Box style={{ transform: 'scale(1.5)', transformOrigin: 'center' }} py="xl">
                  <Barcode value={fullscreenData?.number || ''} width={2} height={100} fontSize={16} />
                </Box>
              )}
            </Box>
